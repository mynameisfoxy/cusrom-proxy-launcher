...
namespace launcherProxy.Views
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            //  Uncomment so see normal exceptions on English
        ...
        private async void ParseVaultRootToken(object sender, TextChangedEventArgs e)
        {
            ...
            {
                VaultRootToken = vaultOutput.Text.Substring(vaultOutput.Text.IndexOf("Root Token:", StringComparison.Ordinal) + 12, 26);
            }

            if (vaultOutput.Text.Contains("Api Address:") && string.IsNullOrEmpty(VaultApiAddress))
            {
                VaultApiAddress = vaultOutput.Text.Substring(vaultOutput.Text.IndexOf("Api Address:", StringComparison.Ordinal) + 13, 21);
            }
        ...
        private void ProxyMoving(bool next = true)
        {
            try
            {
                ...
                if (!File.Exists($"{proxyPath}proxy.exe")) return;
